...
const parseCompoundTExp = (texps: Sexp[]): Result<TExp> => {
    ...
    return (pos === -1)  ? parseTupleTExp(texps) :
           ...
           safe2((args: TupleTExp, returnTE: TExp) => returnProc(args,returnTE))
                (parseTupleTExp(texps.slice(0, pos)), parseTExp(texps[pos + 1]));
};

const returnProc = (args: TupleTExp, returnTE: TExp): Result<TExp> => 
    isNonEmptyTupleTExp(args) ? makeOk(makeProcTExp(args.TEs, returnTE)) : 
    isEmptyTupleTExp(args) ? makeOk(makeProcTExp([],returnTE)) :
    makeFailure("could not make a proper proc exp type");
...
const parseTupleTExp = (texps: Sexp[]): Result<TupleTExp> => {
    ...
    return (isEmptyTuple(texps)) ? makeOk(makeEmptyTupleTExp()) :
     bind(splitEvenOdds(texps),(argTEs: Sexp[]) => {
        const exps:Result<TExp[]> = mapResult(parseTExp, argTEs);
        if (isOk(exps)&& allT(isTExp,exps.value))
        return makeOk(makeNonEmptyTupleTExp(exps.value))
        if(isOk(exps)) return makeFailure("problem with allT")
        return makeFailure("problem with isOk")});
}
...
export const unparseTExp = (te: TExp): Result<string> => {
    const unparseTuple = (paramTes: TExp[]): Result<string[]> =>
        ...
        //isEmptyTupleTExp(paramTes)? makeOk(["empty"]) :
    ...
    const up = (x?: TExp): Result<string | string[]> =>
        ...
        isTupleTExp(x) ? isNonEmptyTupleTExp(x)? unparseTuple(x.TEs): unparseTuple([]) :
