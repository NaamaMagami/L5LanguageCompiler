...
import { isAppExp, isBoolExp, isDefineExp, isIfExp, isLetrecExp, isLetExp, isNumExp,
         ...
         Parsed, PrimOp, ProcExp, Program, StrExp,isValuesExp,isLetValuesExp, valuesExp, LetValuesExp ,CExp, valuesBindingsExp, VarDecl} from "./L5-ast";
import { applyTEnv, makeEmptyTEnv, makeExtendTEnv, TEnv } from "./TEnv";
import { isProcTExp, makeBoolTExp, makeNumTExp, makeProcTExp, makeStrTExp, makeVoidTExp,
         parseTE, unparseTExp,
         BoolTExp, NumTExp,isTExp, StrTExp, TExp, VoidTExp ,makeEmptyTupleTExp, isNonEmptyTupleTExp, makeNonEmptyTupleTExp} from "./TExp";
...
export const typeofValues = (valExp:valuesExp,tenv:TEnv):Result<TExp> =>{
    const expsTypes:Result<TExp[]> = mapResult((v:Exp)=> typeofExp(v, tenv),valExp.vals);
    if (isOk(expsTypes)){
        if(isEmpty(expsTypes.value))
            return makeOk(makeEmptyTupleTExp());
        return allT(isTExp,expsTypes.value)? makeOk(makeNonEmptyTupleTExp(expsTypes.value)) : makeFailure("Invalid type valuesExp");
    }
    return makeFailure("Invalid vals in values Exp");
}
...
export const typeofLetValues = (exp:LetValuesExp,tenv:TEnv):Result<TExp> =>{
    const vars:VarDecl[] = map((b)=>b.vars,exp.bindings).reduce((acc,curr)=>acc=acc.concat(curr),[]);
    const varTEs:TExp[] = map((v:VarDecl)=>v.texp,vars);
    const varsString:string[] = vars.map((v:VarDecl)=>v.var);
    const emptyArr:CExp[]=[];
    const values:CExp[] = map((b:valuesBindingsExp)=>b.values,exp.bindings).reduce
    ((acc,curr)=>isValuesExp(curr)? acc=acc.concat(curr.vals): acc=acc.concat([curr]),emptyArr);
    const constraints = zipWithResult((varTE, val) => bind(typeofExp(val, tenv),
                                                           (typeOfVal: TExp) => checkEqualType(varTE, typeOfVal, exp)),
                                      varTEs, values);
    return bind(constraints, _ => typeofExps(exp.body, makeExtendTEnv(varsString, varTEs, tenv)));
}
...
//   const a1 = L5typeof(`(values 10 3)`);
//     const a2 = L5typeof(`(values 10 (+ 1 2) (> 5 4) #f)`);
//     const a3 = L5typeof(`(lambda ((x : string) (y : string)) : (number * number) (values 1 2))`);
//     const a4 = L5typeof(`(lambda () : (number * number) (values 1 2))`);
//     const a5 = L5typeof(`(let-values ((((x : number) (y : number)) (values 10 3))) (+ x y))`);
//     const a6 = L5typeof(`(let-values ((((n : number) (s : string)) (values 1 "string"))) s)`);
    
//     isOk(a1) ? console.log(a1.value) : console.log(a1.message);
//     isOk(a2) ? console.log(a2.value) : console.log(a2.message);
//     isOk(a3) ? console.log(a3.value) : console.log(a3.message);
//     isOk(a4) ? console.log(a4.value) : console.log(a4.message);
//     isOk(a5) ? console.log(a5.value) : console.log(a5.message);
//     isOk(a6) ? console.log(a6.value) : console.log(a6.message);  
