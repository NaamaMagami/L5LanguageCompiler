...
//         |  ( values <cexp>*)               / values(args:cexp[])
//         |  ( let-values <var-ref>* (values) <cexp>* )
...
//export const isCExp = (x: any): x is CExp => isAtomicExp(x) || isCompoundExp(x) || isTupleExp(x);
...
//export type CompoundExp = AppExp | IfExp | ProcExp | LetExp | LitExp | LetrecExp | SetExp | valuesExp | TupleExp;
...
export const isCompoundExp = (x: any): x is CompoundExp =>
    ...
    //isLetValuesExp(e) ? map((v)=>v.var,e.vars) :  fixxxx
...
//(let-values (valuesBinding) (body) )
export interface LetValuesExp {tag: "LetValuesExp"; bindings:valuesBindingsExp[] ; body: CExp[]; }
export const makeLetValuesExp = (bindings:valuesBindingsExp[], body: CExp[]): LetValuesExp =>
    ({tag: "LetValuesExp", bindings:bindings, body: body});
export const isLetValuesExp = (x: any): x is LetValuesExp => x.tag === "LetValuesExp";

//(valuesBindings (vars) (values) )
export interface valuesBindingsExp {tag: "valuesBindingsExp"; vars:VarDecl[] ;values:CExp }
export const makevaluesBindingsExp = (vars:VarDecl[],values:CExp): valuesBindingsExp =>
    ({tag: "valuesBindingsExp", vars:vars, values:values});
export const isvaluesBindingsExp = (x: any): x is valuesBindingsExp => x.tag === "valuesBindingsExp";
...
//     if(op === "define" ) return parseDefine(params) 
//     else
//     return parseL5CompoundCExp(op, params);
// }
...
bind(mapResult(parseL5CExp, params),
         (cexps: Exp[]) => allT(isCExp,cexps)? makeOk(makeValuesExp(cexps)): makeFailure("values must include onlt Cexps"));
...
const isGoodLetValuesBindings = (bindings: Sexp): bindings is [Sexp[], Sexp][] =>
    isArray(bindings) && allT(isArray, bindings) && allT(isArray, map((b)=>b[0],bindings));
...
! isGoodLetValuesBindings(bindings) ? makeFailure(`Invalid bindings: ${JSON.stringify(bindings)}`) :
safe2((valuesBindings:valuesBindingsExp[], body: CExp[]) => makeOk(makeLetValuesExp(valuesBindings, body)))
    (parseValBindings(bindings), mapResult(parseL5CExp, body));

const parseValBindings = (bindings: [Sexp[],Sexp][]) : Result<valuesBindingsExp[]> =>{
    const varsArr:Sexp[][] = map((b)=>b[0],bindings);
    const varDeclsResultArray = map((b:Sexp[])=>mapResult(parseVarDecl,b), varsArr);
    const varDeclsArray = map((arr)=>isOk(arr)?arr.value:arr,varDeclsResultArray);
    const valuesArray:Result<CExp[]> = mapResult(parseL5CExp,map(b=>b[1],bindings));
     //console.log(valuesArray);
    if (allT(isArray,varDeclsArray) && isOk(valuesArray)){
        return makeOk(zipWith(makevaluesBindingsExp,varDeclsArray,valuesArray.value));
    }
    else return makeFailure("Invalid bindings args");
}
...
export const unparse = (e: Parsed): Result<string> =>
    ...
    // AppExp | IfExp | ProcExp | LetExp | LitExp | LetrecExp | SetExp | ValuesExp | letValuesExp
...
const unparseValuesBindings = (bindings:valuesBindingsExp[]): Result<string> =>
    bind(mapResult(bdg => safe2((vd: string, val: string) => makeOk(`(${vd} ${val})`))(bind(mapResult(unparseVarDecl,bdg.vars),(array:string[])=>makeOk(`(${join(" ", array)})`)), unparse(bdg.values)), bindings),
        (bdgs: string[]) => makeOk(join(" ", bdgs)));
...
const unparseValuesExp = (ve : valuesExp): Result<string> => 
    isEmpty(ve.vals)?  makeOk(`(values)`) :
    bind (mapResult(unparse,ve.vals), (vals:string[])=> makeOk(`(values ${vals.join(" ")})`));
...
//const a1 = parseL5(`(L5 (define f (lambda (x) (values 1 2 (values 1)))))`);
  //const a2 = parseL5(`(L5 (let-values (((x y z) (values 10 3 (values 1 2))) (() (values))) (list y x)))`);
// const a3 = parseL5(`(L5 (let-values (((a b c) (f 0)))(+ a b c)))`);
// const a4 = parseL5(`(L5 (let-values (((n s) (values 1 “string”))) n))`);

 //isOk(a1) ? console.log(unparse(a1.value)) : console.log(a1.message);
 // isOk(a2) ? console.log(unparse(a2.value)) : console.log(a2.message);
// isOk(a3) ? console.log(unparse(a3.value)) : console.log(a3.message);
// isOk(a4) ? console.log(unparse( a4.value)) : console.log(a4.message);
